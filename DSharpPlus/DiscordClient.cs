    ...
    public sealed class DiscordClient : BaseDiscordClient
    {
        ...
        internal async Task OnInvalidateSessionAsync(bool data)
        {
            ...
            else
            {
                ...
                await this.InternalReconnectAsync(true).ConfigureAwait(false);
            }
        }
        ...
        {
            var skipped_heartbeats = Volatile.Read(ref this._skippedHeartbeats);
            var guilds_comp = Volatile.Read(ref this._guildDownloadCompleted);
            if (guilds_comp && skipped_heartbeats > 0)
            {
                this.DebugLogger.LogMessage(LogLevel.Critical, "DSharpPlus", "Client heartbeat wasn't acknowledged. Issuing reconnect.", DateTime.Now);
                await this.InternalReconnectAsync(code: 4001, message: "Too many heartbeats missed").ConfigureAwait(false);
                return;
            }
            else if (!guilds_comp && skipped_heartbeats > 5)
            {
